@extends('layouts.app')

@section('content')
<div class="container-fluid d-flex justify-content-between align-items-center py-3 px-5 border-3 border-bottom rounded-3">
  <h4 class="fw-semibold mb-0">{{ isset($status) ? 'Edit Status' : 'Tambah Status Master' }}</h4>
  <a href="{{ route('status_masters.index') }}" class="btn btn-secondary">Kembali</a>
</div>

<div class="container mt-4 bg-white p-4 rounded-3 shadow-sm" style="max-width: 600px;">
  @include('layouts.flash-message')
  <form action="{{ isset($status) ? route('status.update', $status->id) : route('status.store') }}" method="POST">
    @csrf
    @if(isset($status))
      @method('PUT')
    @endif
    <div class="mb-3">
      <label for="nama_status" class="form-label fw-semibold">Nama Status</label>
      <input type="text" name="nama_status" id="nama_status" class="form-control @error('nama_status') is-invalid @enderror"
             placeholder="Masukkan nama status"
             value="{{ old('nama_status', $status->nama_status ?? '') }}" required>
      @error('nama_status')
        <div class="invalid-feedback">{{ $message }}</div>
      @enderror
    </div>
    <div class="mb-3">
      <label for="deskripsi" class="form-label fw-semibold">Deskripsi</label>
      <textarea name="deskripsi" id="deskripsi" class="form-control" placeholder="Tuliskan deskripsi status (opsional)">{{ old('deskripsi', $status->deskripsi ?? '') }}</textarea>
    </div>
    <div class="d-flex justify-content-end gap-2">
      <button type="reset" class="btn btn-danger">Reset</button>
      <button type="submit" class="btn btn-success">{{ isset($status) ? 'Update' : 'Simpan' }}</button>
    </div>
  </form>
</div>
@endsection
